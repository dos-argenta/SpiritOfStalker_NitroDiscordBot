    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator and Checker - Made by DOS×ARGENTA#9999")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by DOS×ARGENTA#9999\a',
 ...
 \ \_____  \   \ \  \ \ \   __  \ \  \    \ \   ___  \ \  \_|/_\ \   _  _\      
  \|____|\  \   \ \  \ \ \  \ \  \ \  \____\ \  \\ \  \ \  \_|\ \ \  \\  \|     
   ...
   \|_________|
        ...
        self.slowType("Dev by: DOS×ARGENTA#9999", .02)
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by DOS×ARGENTA#9999")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by DOS×ARGENTA#9999\a', end='', flush=True)
        ...
        if response.status_code == 200:  # If the responce went through
            ...
            if notify is not None:  # If a webhook has been added
                DiscordWebhook(  # Send the message to discord letting the user know there has been a valid nitro code
                    url=url,
                    content=f"NITRO VALIDE DÉTECTER !!! @everyone \n{nitro}"
                ).execute()
